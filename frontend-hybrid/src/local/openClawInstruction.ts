export function buildOpenClawInstruction(apiKey: string, isChinese: boolean, origin = window.location.origin) {
    if (isChinese) {
        return `请在 OpenClaw Agent 中创建技能文件 skills/digitalemployee_sync.md，内容如下；然后把「使用 digitalemployee_sync 技能检查数字员工 inbox 并处理待办消息」加入 HEARTBEAT.md：
