...
async function request<T>(url: string, options: RequestInit = {}): Promise<T> {
    ...
    // 数字员工本地壳层：FormData 不设 Content-Type，浏览器自动补 multipart boundary
...
export const agentApi = {
    ...
    // 数字员工本地壳层：agent 头像上传/删除（本地后端 /agents/{id}/avatar）
...
export const fileApi = {
    ...
    // 数字员工本地壳层：技能 ZIP 导入（本地后端 /agents/{id}/files/import-zip）
...
export const skillApi = {
    ...
    // 数字员工本地壳层：技能 ZIP 导入（本地后端 /skills/import-zip）
