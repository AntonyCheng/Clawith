...
import AgentProfileCard from '../../local/components/AgentProfileCard';
...
export default function AgentDetailPage() {
    ...
    const ChatMessageItem = React.useMemo(() => React.memo(({
        msg, i, isLeft, t, senderLabel, avatarText, avatarUrl, forceSenderLabel = false, hideAvatar = false, hideDistill = false,
    }: {
        msg: any;
        i: number;
        isLeft: boolean;
    ...
    // 数字员工本地壳层：技能切换器插入（来自 frontend-local，斜杠命令约定）
    ...
    const renderAgentInfoCard = () => (
        <AgentProfileCard
            ...
            showToolsSkills={isChatRoute && !!id}
        />
    );
                        ...
                        const filterBtn = (key: string, label: React.ReactNode, indent = false) => (
                            <button
                                ...
                                style={{
                                    ...
                                    color: logFilter === key ? '#ffffff' : 'var(--text-secondary)',
                                    background: logFilter === key ? '#dc2626' : 'transparent',
                                    border: logFilter === key ? '1px solid #dc2626' : '1px solid var(--border-subtle)',
                                    borderRadius: '6px',
                                    cursor: 'pointer',
                                    transition: 'all 0.15s',
