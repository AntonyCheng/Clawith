...
const Login = lazy(() => import('./local/pages/Login'));
const ForgotPassword = lazy(() => import('./local/pages/ForgotPassword'));
const ResetPassword = lazy(() => import('./pages/ResetPassword'));
const VerifyEmail = lazy(() => import('./pages/VerifyEmail'));
const CompanySetup = lazy(() => import('./local/pages/CompanySetup'));
const Onboarding = lazy(() => import('./local/pages/Onboarding'));
const Layout = lazy(() => import('./pages/Layout'));
// 数字员工本地壳层路由语义（2026-08-30 对齐上游：消息广场已移除）：
//   /plaza = 经验库（上游 Plaza 原生语义），/experience → 重定向 /plaza（兼容期），
//   /dashboard = 本地数字员工分析仪表盘，首页落 /dashboard（上游同款）。
const LocalDashboard = lazy(() => import('./local/pages/Dashboard'));
const Roster = lazy(() => import('./local/pages/Roster'));
        ...
        <>
            ...
            <Routes>
                ...
                <Route path="/" element={<ProtectedRoute><Layout /></ProtectedRoute>}>
                    ...
                    <Route path="experience" element={<Navigate to="/plaza" replace />} />
