...
export default function ToolsManager({ agentId, canManage = false }: { agentId: string; canManage?: boolean }) {
    ...
    // Smithery 托管 MCP 工具的授权状态机（v1.11.4）：按分组 key 记录，按钮点击时查询/跳转授权。
    ...
    const checkMcpAuthorization = async (groupKey: string, groupTools: any[]) => {
        ...
        // Authorize/Re-authorize 在点击手势仍有效时预开弹窗；纯 Check 不闪空窗，
        // 若发现需授权则走可靠的 same-page 降级。
                                ...
                                {canManage && smitheryAuthorizationTool && (
                                    <>
                                        ...
                                        <button
                                            ...
                                            style={{ background: '#FFFFFF', border: '1px solid var(--border-subtle)', borderRadius: '6px', padding: '4px 8px', fontSize: '11px', cursor: checkingAuthorization ? 'wait' : 'pointer', color: 'var(--text-secondary)', opacity: checkingAuthorization ? 0.65 : 1 }}
